...
skinparam rectangle<<workflow>> {
    BackgroundColor #e1f5fe
    BorderColor #01579b
    BorderThickness 2
}

skinparam file<<artifact>> {
...
' CHANGED: Updated from diamond to rectangle with specific styling
skinparam rectangle<<decision>> {
    BackgroundColor #fce4ec
    BorderColor #880e4f
    BorderThickness 2
    ' Note: We cannot force a diamond shape easily in this diagram type, 
    ' so we use a distinct color/style to represent the decision.
}
...
' --- PHASE 1: INIT ---
package "Phase 1: Analysis" as P1 {
    rectangle "workflow-init" as W_Init <<workflow>>
    rectangle "product-brief" as W_Brief <<workflow>>
    file "bmm-workflow-status.yaml" as A_Status <<artifact>>
    file "product-brief.md" as A_Brief <<artifact>>
...
' --- PHASE 2: PLANNING ---
package "Phase 2: Planning" as P2 {
    rectangle "create-prd" as W_PRD <<workflow>>
    file "prd.md" as A_PRD <<artifact>>
    
    W_PRD --> A_PRD
}

' --- PHASE 3: SOLUTION ---
package "Phase 3: Solutioning" as P3 {
    rectangle "create-architecture" as W_Arch <<workflow>>
    file "architecture.md" as A_Arch <<artifact>>
    rectangle "create-epics" as W_Epics <<workflow>>
    file "epics.md" as A_Epics <<artifact>>
    
    ' FIXED: Changed 'diamond' to 'rectangle'
    rectangle "Ready?" as Check_Ready <<decision>>

    W_Arch --> A_Arch
    A_Arch --> W_Epics
    W_Epics --> A_Epics
    A_Epics --> Check_Ready
    Check_Ready -up-> W_Arch : No
}

' --- PHASE 4: IMPL (NESTED LOOPS) ---
package "Phase 4: Implementation Loops" as P4 {
    
    rectangle "sprint-planning" as W_SprintPlan <<workflow>>
    file "sprint-status.yaml" as A_Sprint <<artifact>>
    rectangle "test-design" as W_TestDes <<workflow>>
    file "test-design.md" as A_Test <<artifact>>

    package "Story Execution Loop" as StoryLoop {
        rectangle "create-story" as W_CreateStory <<workflow>>
        file "Story.md" as A_Story <<artifact>>
        rectangle "dev-story" as W_Dev <<workflow>>
        rectangle "code-review" as W_Review <<workflow>>
        
        ' FIXED: Changed 'diamond' to 'rectangle'
        rectangle "Next Story?" as Check_MoreStories <<decision>>

        W_CreateStory --> A_Story
        A_Story --> W_Dev
        W_Dev --> W_Review
        W_Review -left-> W_Dev : Fix
        W_Review --> Check_MoreStories : OK
        Check_MoreStories -up-> W_CreateStory : Yes
    }

    rectangle "epic-retrospective" as W_Retro <<workflow>>
    file "epic-retro.md" as A_Retro <<artifact>>
    
    ' FIXED: Changed 'diamond' to 'rectangle'
    rectangle "Next Epic?" as Check_MoreEpics <<decision>>

    ' Internal Wiring
    W_SprintPlan --> A_Sprint
    A_Sprint --> W_TestDes
    W_TestDes --> A_Test
    A_Test --> W_CreateStory
    
    Check_MoreStories --> W_Retro : No
    
    W_Retro --> A_Retro
    A_Retro --> Check_MoreEpics
    Check_MoreEpics -up-> W_SprintPlan : Yes
}

circle "Done" as Finish <<endstate>>

' --- EXCEPTIONS ---
package "Exceptions" as POther {
    rectangle "party-mode" as W_Party <<other>>
    rectangle "quick-dev" as W_Quick <<other>>
    rectangle "correct-course" as W_Correct <<other>>
    file "change-proposal.md" as A_Change <<artifact>>
...
A_Brief --> W_PRD
A_PRD --> W_Arch
Check_Ready --> W_SprintPlan : Yes
Check_MoreEpics --> Finish : No
...
System_Boundary(c1, "Coretext System") {
    Container(cli_tool, "Coretext CLI", "Python / Typer", "The primary command-line tool. Handles lifecycle (init, start), status checks, and direct graph operations.")
    
    Container(gemini_extension, "Gemini CLI Extension", "Python Adapter", "The client adapter. Registers 'gemini coretext' commands and proxies MCP requests to the daemon.")
...
Rel(developer, cli_tool, "2. Runs commands (init, status, lint)", "Terminal/stdio")

Rel(gemini_cli, gemini_extension, "3. Loads extension & discovers MCP")
Rel(gemini_extension, daemon, "4. Queries semantic context via Adapter", "HTTP (localhost)")
...
Rel(cli_tool, daemon, "Checks health/status", "HTTP (localhost)")
...
### **3.3.3. The Augmented Workflow**

To bridge the "Topological Autonomy Gap," the standard BMad workflow (Figure 2.1) is augmented with specific **Synchronization** and **Retrieval** checkpoints. This revised workflow ensures that the agent always operates on a synchronized graph state and proactively queries that state before executing complex tasks.

**Figure 3.5** illustrates the Coretext-Augmented BMad Workflow, highlighting the injection of "Sync" and "Query" operations into the implementation loop.
...
' --- PHASE 1-3: PLANNING ---
package "Phases 1-3: Specification" as P123 {
    rectangle "Planning Workflows" as W_Plan <<workflow>>
    file "Specs\n(PRD, Arch, Epics)" as A_Specs <<artifact>>
    
    W_Plan --> A_Specs
}

' --- CORETEXT SYNC ---
rectangle "<b>Coretext Sync</b>\n(Ingest Planning Artifacts)" as Sync_Specs <<coretext>>
...
package "Phase 4: Implementation Loops" as P4 {
    ...
    package "Story Execution Loop" as StoryLoop {
        rectangle "<b>Query Knowledge</b>\n(Context for Story)" as Query_Create <<coretext>>
        rectangle "create-story" as W_CreateStory <<workflow>>
        file "Story.md" as A_Story <<artifact>>
        rectangle "<b>Coretext Sync</b>\n(Ingest Story)" as Sync_Story <<coretext>>
        
        rectangle "<b>Query Knowledge</b>\n(Context for Dev)" as Query_Dev <<coretext>>
        rectangle "dev-story" as W_Dev <<workflow>>
        
        rectangle "<b>Query Knowledge</b>\n(Context for Review)" as Query_Review <<coretext>>
        rectangle "code-review" as W_Review <<workflow>>
        
        rectangle "Next Story?" as Check_MoreStories <<decision>>

        Query_Create --> W_CreateStory
        W_CreateStory --> A_Story
        A_Story --> Sync_Story
        Sync_Story --> Query_Dev
        Query_Dev --> W_Dev
        W_Dev --> Query_Review
        Query_Review --> W_Review
        W_Review -left-> Query_Dev : Fix
        W_Review --> Check_MoreStories : OK
        Check_MoreStories -up-> Query_Create : Yes
    }

    rectangle "<b>Query Knowledge</b>\n(Context for Retro)" as Query_Retro <<coretext>>
    rectangle "epic-retrospective" as W_Retro <<workflow>>
    file "epic-retro.md" as A_Retro <<artifact>>
    rectangle "<b>Coretext Sync</b>\n(Ingest Retro)" as Sync_Retro <<coretext>>
    
    rectangle "Next Epic?" as Check_MoreEpics <<decision>>

    ' Connections
    W_SprintPlan --> Query_Create
    
    Check_MoreStories --> Query_Retro : No
    
    Query_Retro --> W_Retro
    W_Retro --> A_Retro
    A_Retro --> Sync_Retro
    Sync_Retro --> Check_MoreEpics
    Check_MoreEpics -up-> W_SprintPlan : Yes
}
...
While the current system has achieved its primary utility goals, the path to fully autonomous software engineering requires addressing specific technical limitations identified during the Bootstrapping phase.
