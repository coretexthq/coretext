...
def main():
    ...
    success, err = engine.add_rules(args.source, args.target, args.type, args.description)
    if success:
        print(f"Successfully added edge: {args.source} [{args.description}] -> {args.target} ({args.type})")
    else:
