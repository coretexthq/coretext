...
4. **Context Injection:** A deterministic engine passively injects `docs/` and `docs/rules/` files directly into an agent's context based on glob paths defined in `.coretext/{workspace_name}.jsonl`. This structural state, alongside real-time telemetry of active agent file interactions, can be visualized interactively by running the local `.coretext/coretext-graph-ui` dashboard, which supports dynamic graph and session selection.
