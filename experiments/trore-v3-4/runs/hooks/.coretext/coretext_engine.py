...
class CoretextEngine:
    def __init__(self, coretext_dir: str):
        ...
        self.data_dir = self.workspace_root / ".coretext-data"
        self.jsonl_path = self.data_dir / f"{workspace_name}_rules.jsonl"
        if not self.jsonl_path.exists() and self.data_dir.exists():
            candidate_ledgers = list(self.data_dir.glob("*_rules.jsonl"))
            if candidate_ledgers:
                self.jsonl_path = candidate_ledgers[0]
