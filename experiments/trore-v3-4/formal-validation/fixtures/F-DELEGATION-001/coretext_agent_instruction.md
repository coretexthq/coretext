...
1. `.coretext-data/rules/`
2. `docs/`
3. durable notes in `knowledge/` outside `knowledge/ai/`
4. repository code and Git state
...
- A leaf or delegated worker must not modify durable notes in `knowledge/`, parent notes, sibling notes, or `.coretext-data/rules/` unless the parent explicitly assigns it an integration role.
...
Durable notes may contain at most two non-frontmatter `---` separators. Do not use horizontal rules inside the body.
...
Automatic conversion into graph-routed tests, hooks, linters, or other enforcement mechanisms is future implementation unless the repository already provides explicit tooling and the parent agent deliberately uses it. Do not invent enforcement work as part of an ordinary feature task.

### When a Rule Is Appropriate

A rule may be created only when all of these are true:
...
Leaf or delegated workers do not create rules. They may recommend a possible rule in their session summary. The parent or integrating agent decides after review.

### Pre-Work Graph Check

Before creating a rule or registering a graph edge, run:
...
Review standalone rule warnings before adding more rules.

### New Rule File

If the parent or integrating agent confirms that a new atomic rule is needed, create:

```text
.coretext-data/rules/<rule-name>.md
```
...
```markdown
---
session: knowledge/ai/<namespace>.<session-name>.md
---

# Short Title

**Trigger:** Goal or source context that activates the rule.

## Context
What was attempted and why the rule exists.

## Axiom
The hard architectural rule future agents must follow.
```

### Graph Edge

If a rule should be activated by Coretext routing, register it with:
