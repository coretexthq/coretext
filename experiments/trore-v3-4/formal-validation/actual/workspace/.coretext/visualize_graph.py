...
def generate_mermaid(jsonl_path):
    ...
    def get_category(path):
        ...
        if path.startswith(".coretext-data/rules/"): return "rules"
        if path.startswith("docs/templates/"): return "templates"
        if path.startswith("docs/"): return "docs"
        if path.endswith("_rules.jsonl"): return "coretext"
...
if __name__ == "__main__":
    ...
    workspace_name = engine.workspace_root.name
    jsonl_path = engine.jsonl_path
    output_path = engine.data_dir / "graph_rules.md"
    ...
    content = [
        ...
        "",
        f"This graph visualizes the structural context injection edges defined in `{jsonl_path.name}`.",
        "",
        "```mermaid",
